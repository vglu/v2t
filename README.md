...
| **Установщик Windows (NSIS)** | `src-tauri/target/release/bundle/nsis/Video to Text_1.1.1_x64-setup.exe` (имя зависит от `version` в `tauri.conf.json`) |
| **Установщик Windows (MSI)** | `src-tauri/target/release/bundle/msi/Video to Text_1.1.1_x64_en-US.msi` |
...
| **Release** (`.github/workflows/release.yml`) | push **git-тега** вида `v*` (например `v1.1.1`) | Сборка **Windows** (NSIS/MSI), **macOS** (**aarch64** и **x86_64** DMG), **Linux** (.deb + AppImage), загрузка в [GitHub Releases](https://docs.github.com/repositories/releasing-projects-on-github/about-releases) |
...
3. Создайте и запушьте тег: `git tag v1.1.1` → `git push origin v1.1.1`.
