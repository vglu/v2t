...
| **Установщик Windows (NSIS)** | `src-tauri/target/release/bundle/nsis/Video to Text_1.1.2_x64-setup.exe` (имя зависит от `version` в `tauri.conf.json`) |
| **Установщик Windows (MSI)** | `src-tauri/target/release/bundle/msi/Video to Text_1.1.2_x64_en-US.msi` |
...
3. Создайте и запушьте тег: `git tag v1.1.2` → `git push origin v1.1.2`.
