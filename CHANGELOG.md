...
- **Плейлисты: отдельный файл на каждый ролик.** При шаблоне без `{track}` (например `{title}_{date}.txt`) все треки плейлиста перезаписывали один и тот же `.txt`/`.m4a`; resume пропускал треки 2…N как «уже готово». Если в шаблоне нет `{track}`, а в задаче несколько роликов, к имени автоматически добавляется `_t1`, `_t2`, …
